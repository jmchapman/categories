...
rightlaw' {X = X}{Y}{f} = AlgMorphEq' 
  ...
  refl 
  (trans 
    (cong (λ (f₁ : Hom C X (OMap (R (adj A)) Y)) → HMap (R (adj A)) 
                                                        (right (adj A) f₁))
          (sym (stripsubst (Hom C X) 
                           f 
                           (fcong Y (cong OMap Rlaw'))))) 
    (sym (stripsubst (λ Z → Hom C Z (OMap (R (adj A)) Y))
                     ((HMap (R (adj A)) (right (adj A) 
                                               (subst (Hom C X) 
                                                      (fcong Y (cong OMap 
                                                                     Rlaw')) 
                                                      f))))
