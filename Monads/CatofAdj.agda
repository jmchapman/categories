...
record ObjAdj {c d} : Set (a ⊔ b ⊔ suc c ⊔ suc d) where
  field D   : Cat {c}{d}
        adj : Adj C D
  open Adj adj
...
idHomAdj {X = X} = record {
    K = IdF (D X);
    Llaw = FunEq refl λ _ → refl;
    Rlaw = FunEq refl λ _ → refl;
    rightlaw = rightlawlem R
                           L
                           (cong OMap (FunEq refl (λ _ → refl))) 
                           right}
...
idlHomAdj{X = X}{Y = Y}{f = f} = 
  HomAdjEq _ _ (FunEq refl (λ {X}{Y} h → refl))
...
assHomAdj {W = W}{X = X}{Y = Y}{Z = Z}{f = f}{g = g}{h = h} = 
  HomAdjEq _ _ (FunEq refl (λ h → refl))

CatofAdj : ∀{c d} → Cat {a ⊔ b ⊔ suc c ⊔ suc d}{a ⊔ b ⊔ c ⊔ d}
CatofAdj {c}{d} = record {
         Obj  = ObjAdj {c}{d};
  Hom  = HomAdj;
  iden = idHomAdj;
